package com.mrii.prodes.viewobject.holder;
...
public class ItemParameterHolder implements Serializable {
    ...
    public String keyword, search_type, item_id, cat_id, sub_cat_id,type_id,price_type_id,condition_id,deal_option_id,currency_id,location_id,
            type_name,price_type_name,condition_name,deal_option_name,currency_name,location_name,
            order_by, order_type, lat, lng, mapMiles,max_price, min_price,userId,isPaid, cityLat, cityLng, status, provinsi_id, kabupaten_kota_id, kecamatan_id, kelurahan_desa_id;
...
//        }
        ...
        if (!provinsi_id.isEmpty()){
            result += provinsi_id + ":";
        }
        if (!kabupaten_kota_id.isEmpty()){
            result += kabupaten_kota_id + ":";
        }
        if (!kecamatan_id.isEmpty()){
            result += kecamatan_id + ":";
        }
        if (!kelurahan_desa_id.isEmpty()){
            result += kelurahan_desa_id + ":";
        }
        return result;
    }
}
