package com.mrii.prodes.ui.user.phonelogin;
...
public class PhoneLoginFragment extends PSFragment {
    ...
    protected void initUIAndActions() {
        ...
        //dataBindingComponent.getFragmentBindingAdapters().bindFullImageDrawable(binding.get().bgImageView, getResources().getDrawable(R.drawable.img_number_verification));
...
//            }else if
            if(binding.get().phoneEditText.getText().toString().trim().isEmpty()){
                ...
                number = binding.get().phoneCodeEditText.getText()+binding.get().phoneEditText.getText().toString();
                //userName = "";  //binding.get().nameEditText.getText().toString();
                PhoneLoginFragment.this.validNo(number);

                Utils.navigateAfterPhoneVerify(getActivity(),navigationController,number);
    ...
    private void validNo(String no) {
        if (no.isEmpty() || no.length() < 10) {
            binding.get().phoneEditText.setError("Masukan nomor yang benar");
